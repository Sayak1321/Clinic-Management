      ...
      <div className="max-w-7xl w-full mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          {active ? (
            ...
            /* Caseload & Daily Report view (directly in dashboard right column) */
            <div className="card flex-1 flex flex-col gap-4">
              <div className="border-b border-surface-200 pb-3 flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-bold text-slate-800">My Daily Caseload & Summary</h2>
                  <p className="text-xs text-slate-500">Real-time statistics of patients assigned to you today ({today})</p>
                </div>
                <button
              ...
              <div className="grid grid-cols-4 gap-4 text-center">
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 shadow-sm">
                  <p className="text-2xl font-bold text-slate-800">{caseloadTokens.length}</p>
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Total Assigned</p>
                </div>
                <div className="bg-green-50 p-3 rounded-xl border border-green-100 shadow-sm">
                  <p className="text-2xl font-bold text-green-700">{caseloadTokens.filter(t => t.status === 'done').length}</p>
                ...
                <div className="bg-blue-50 p-3 rounded-xl border border-blue-100 shadow-sm">
                  <p className="text-2xl font-bold text-blue-700">{caseloadTokens.filter(t => t.status === 'in_progress').length}</p>
                  <p className="text-[10px] text-blue-600 uppercase tracking-wider font-semibold">In Progress</p>
                </div>
                <div className="bg-amber-50 p-3 rounded-xl border border-amber-100 shadow-sm">
                  <p className="text-2xl font-bold text-amber-700">{caseloadTokens.filter(t => t.status === 'waiting').length}</p>
                  <p className="text-[10px] text-amber-600 uppercase tracking-wider font-semibold">Waiting</p>
                </div>
              </div>

              {/* Caseload Remarks Textarea */}
              <div className="flex flex-col gap-1.5">
                <label className="block text-xs font-semibold text-slate-700 font-medium">General Notes / Daily Report Comments</label>
                <textarea
                  value={caseloadNotes}
                  onChange={e => setCaseloadNotes(e.target.value)}
                  className="input"
                  rows={3}
                  placeholder="Write a brief general report summary on today's cases (this will be exported in your report download)..."
                />
              </div>

              {/* Patient caseload list */}
              <div className="flex-1 flex flex-col min-h-[220px]">
                <h3 className="text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Patients Summary List</h3>
                <div className="border border-slate-200 rounded-xl overflow-y-auto max-h-[300px] flex-1 divide-y divide-slate-100 bg-white">
                  {caseloadTokens.map((t, idx) => (
                    <div key={t.id} className="p-3 flex justify-between items-center hover:bg-slate-50 transition-colors text-xs">
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-slate-800">Token #{t.token_number} — {t.patientName}</p>
                          <span className="text-[10px] text-slate-400">({t.patientPhone})</span>
                        </div>
                        <p className="text-[10px] text-slate-500 mt-0.5">Complaint: {t.chiefComplaint}</p>
                        {t.status === 'done' && (
                          <p className="text-[10px] text-primary-700 font-semibold mt-0.5 bg-primary-50 px-1.5 py-0.2 rounded inline-block">
                  ...
                  {caseloadTokens.length === 0 && (
                    <div className="text-slate-400 text-center py-12 flex flex-col items-center justify-center h-full">
                      ...
                      <p className="text-xs">No patients assigned to you today.</p>
                    </div>
                  )}
                </div>
              </div>
